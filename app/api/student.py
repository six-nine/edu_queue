...
class Student:
    ...
    def join_group(self, invite_code: str) -> None:
        bool_result = self.database.join_group(invite_code=invite_code, student_id=self.student_tg_id)
        if not bool_result:
            raise StudentJoinGroupException(self.student_tg_id, invite_code)


    def leave_group(self, invite_code: str) -> None:
        bool_result = self.database.quit_group(invite_code=invite_code, student_id=self.student_tg_id)
        if not bool_result:
